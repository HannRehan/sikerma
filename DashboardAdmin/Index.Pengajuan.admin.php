...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($nama_pengusul) || empty($email_pengusul) || empty($lembaga_mitra) || empty($judul) || empty($tingkat)) {
        echo "<script>alert('Semua formulir harus diisi');window.location.href = 'index.Pengajuan.admin.php'</script>";
        exit;
    }

    // Validasi format email menggunakan filter_var
    if (!filter_var($email_pengusul, FILTER_VALIDATE_EMAIL)) {
        echo "<script>alert('Format email tidak valid');window.location.href = 'index.Pengajuan.admin.php'</script>";
        exit;
    }
    ...
    if (mysqli_query($conn, $sql)) {
        ...
        header("Location: ProsesUsulanAdmin.php?success=true");
        exit;
    } else {
...
<!-- endnavbar -->
    ...
    <input type="checkbox" id="status" name="status" value="1" checked disabled>
            ...
            <a href="../DashboardAdmin/index_admin.php" class="btn btn-danger">
            CANCEL
            </a>
            </div>
